...
unlink $file;
ok !-e $file;
my $write_ok = NHGRI::Db::Connector->write_realm_file(
...
$NHGRI::Db::Connector::PRIVATE_CONNECTION_DIR = $test_dir;
unlink $file;
ok !-e $file;
$write_ok = NHGRI::Db::Connector->write_realm_file(
...
my $sh_dir = "t/shared";
mkdir $sh_dir, 0755;
$NHGRI::Db::Connector::SHARED_CONNECTION_DIR = $sh_dir;
...
$dbc = NHGRI::Db::Connector->new( -realm => $test_realm );
my $rh = $dbc->read_realm_file();
ok $rh->{'DBD'}, "Mybase";

# Read with static method calls
$rh = NHGRI::Db::Connector->read_realm_file( -filename => $sh_file );
ok $rh->{'DBD'}, "Shmybase";

# Reads private version if it exists
$rh = NHGRI::Db::Connector->read_realm_file( -realm => $test_realm );
ok $rh->{'DBD'}, "Mybase";
